...
def derive_key_and_iv(password, salt, key_length, iv_length):
    ...
    while len(d) < key_length + iv_length:
        ...
        d_i = hashlib.pbkdf2_hmac('sha256', d_i + password.encode() + salt, salt, 100000)
        d += d_i
...
if not os.path.exists('database.db.enc'):
    ...
    cursor = db.cursor()
    cursor.execute("CREATE TABLE websites (website text, username text, password text)")
else:
...
def add_entry(db, website, username, password):
    ...
    if entry:
        cursor.execute("UPDATE websites SET username=?, password=? WHERE website=?", (username, password, website))
    # If the entry does not exist, insert it
    else:
        cursor.execute("INSERT INTO websites (website, username, password) VALUES (?, ?, ?)", (website, username, password))
...
while True:
    ...
    match command:
        ...
        case "unlock":
            if(locked == False): print("The database is not encrypted.")
            else:
                decrypt_file(maskpass.askpass("Enter password to decrypt: "))
                locked = False
        # Lock command to encrypt database
        case "lock":
            if(locked == True): print("The database is already encrypted.")
            else:
            ...
            if(locked == True):
                ...
                # If the list_websites function returned a list, print the websites
                    ...
                    for website in result:
                        number_of_sites += 1
                        print(website)
                    print("\nThere are {} credentials in the database.".format(str(number_of_sites)))
